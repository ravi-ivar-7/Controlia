...
const runShellFile = async (io, socket, data) => {
    ...
    try {
        ...
        let exec = await container.exec({
            Cmd: ['sh', `${SCRIPT_DIR}/${script.scriptName}`, ...args],
            AttachStdin: true,
            AttachStdout: true,
            AttachStderr: true,
            Tty: true,
        ...
        io.to(socket.decodedToken.userId).emit('success', { message: 'Shell script executed successfully.' });

    } catch (error) {
        logger.error(`ERROR IN EXECUTING SHELL SCRIPT: ${error}`);
