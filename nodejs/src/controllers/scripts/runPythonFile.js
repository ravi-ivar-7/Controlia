...
const runPythonFile = async (io, socket, data) => {
    ...
    try {
        ...
        let exec = await container.exec({
            Cmd: ['python3', `${SCRIPT_DIR}/${scriptData.scriptName}`, ...args],
            AttachStdin: true,
            AttachStdout: true,
            AttachStderr: true,
            Tty: true,
        ...
        io.to(socket.decodedToken.userId).emit('success', { message: 'Python script executed successfully.' });

    } catch (error) {
        logger.error(`ERROR IN EXECUTING PYTHON SCRIPT: ${error}`);
